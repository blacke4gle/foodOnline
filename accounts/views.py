...
def registerUser(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 'Your account has ben registered successfully!')
    ...
    if request.user.is_authenticated:
        ...
        if form.is_valid() and v_form.is_valid:
            ...
            send_verification_email(
                request, user, mail_subject, email_template)
            messages.success(
                request, 'Your account has ben registered sucessfully! Please wait for the approval.')
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        user.save()
        messages.success(
            request, "Congratualation! Your account is activated.")
        return redirect('myAccount')
    else:
